...
```bash
# 拉取最新镜像
docker pull crpi-hocnvtkomt7w9v8t.cn-beijing.personal.cr.aliyuncs.com/byte01001/xiaohongshu-mcp
```
...
`byte01001/xiaohongshu-mcp`为镜像名称和版本。
...
> ```yaml
> # image: byte01001/xiaohongshu-mcp
> image: crpi-hocnvtkomt7w9v8t.cn-beijing.personal.cr.aliyuncs.com/byte01001/xiaohongshu-mcp
> ```
...
# 查看日志
docker logs -f byte01001/xiaohongshu-mcp
...
```bash
docker run -e XHS_PROXY=http://user:pass@proxy:port byte01001/xiaohongshu-mcp
```
