...
如果您在部署传统 Docker 版本时遇到问题，**务必先查看：[各种疑难杂症 (Issues #56)](https://github.com/byte01001/xiaohongshu-mcp/issues/56)**。

> *提示：如果环境排查太耗时，切换到 [x-mcp 插件版](https://github.com/byte01001/x-mcp) 通常是更高效的选择。*
...
3. 如果账号没有实名认证，特别是新号，一般会触发 **实名认证** 的消息提醒（参见下图）。⚠️ 这个不是封号，不用 MCP 也会要求实名认证。实名认证后，账号就正常了。建议使用该项目前就先实名，该项目是基于学习的目的，禁止一切违法行为。
...
直接从 [GitHub Releases](https://github.com/byte01001/xiaohongshu-mcp/releases) 下载对应平台的二进制文件：
...
Docker Hub 地址：[https://hub.docker.com/r/byte01001/xiaohongshu-mcp](https://hub.docker.com/r/byte01001/xiaohongshu-mcp)
